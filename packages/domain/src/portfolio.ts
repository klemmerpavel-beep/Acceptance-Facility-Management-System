...
/* --- Объект без сметы --------------------------------------------------------
   Ответ заказчика на вопрос 8 квиза от 14.09.2026: объект без сметы —
   незавершённое заведение, и продукт обязан называть это словом, а не
   молчать. До ответа продукт молчал: на стенде объекты без сметы ничем не
   отличались от объектов со сметой, и недоведённое заведение укрывалось.   */

/**
 * Пометка «сметы нет» на объекте.
 *
 * Выводится из итога действующей сметы, а не из отдельного поля. Второе поле
 * означало бы два источника одного утверждения, и они разошлись бы на первой
 * же правке: смету сняли, поле осталось — и продукт сообщает о смете,
 * которой нет.
 *
 * Ноль отсутствием не считается. Смета на ноль рублей заведена: у неё есть
 * редакция, позиции и автор — это случай 1 словаря пустоты
 * (`apps/web/src/empty.ts`), печатаемый числом, а не случай 2, печатаемый
 * словом. Отождествить их значит объявить заведённую смету незаведённой и
 * потребовать работы, которая уже сделана.
 *
 * Носитель величины различать незачем: на сервере она приходит копейками,
 * на клиенте — строкой копеек, а вопрос у пометки один и тот же.
 */
export function безСметы(estimateTotal: Kopecks | string | null): boolean {
  return estimateTotal === null;
}
