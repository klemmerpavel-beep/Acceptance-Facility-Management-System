...
/**
 * Обратное количество — примитив сторно приёмки.
 *
 * Именованный тип нельзя отрицать напрямую: линт с учётом типов отвергает
 * унарный минус на брендированном `bigint`. Обход написан один раз здесь,
 * а не повторяется приведением на каждом месте вызова.
 */
export const negateQuantity = (a: Milliunits): Milliunits => -(a as bigint) as Milliunits;
