...
/** Первая группа совпадения. Через функцию — иначе на каждом месте стоит
 *  утверждение о непустоте, которого строгий режим требует, а читатель нет. */
const pair1 = (м: RegExpMatchArray): string => м[1] ?? "";
...
describe("иконки", () => {
  ...
  it("типографские знаки и эмодзи не используются в роли иконок", () => {
    ...
    //
    // Буквы, цифры и подстановки значений из-под правила выведены: правило о
    // глифе, подменяющем иконку, а не о всяком скрытом от озвучивания тексте.
    // Декоративное повторение кода объекта на подложке — не глиф: оно скрыто
    // затем, что то же самое уже названо бейджем рядом, и озвучивать его
    // дважды незачем. Прежняя запись ловила и его, то есть стерегла не то,
    // что объявлено в её собственном заголовке.
    const glyphAsIcon = /aria-hidden="true"\s*>\s*[^<\s][^<]*</g;
    const глифом = (кусок: string): boolean => {
      const текст = кусок.slice(кусок.indexOf(">") + 1, -1).trim();
      return !/[\p{L}\p{N}{]/u.test(текст);
    };
    const found = (markup().match(glyphAsIcon) ?? []).filter(глифом);
 ...
 */
/**
 * Прозрачность не расходует контраст.
 *
 * Проверка контраста сравнивает пару токенов и потому слепа к тому, что видно
 * на экране: цвет под `opacity` и цвет на полупрозрачной заливке доходят до
 * глаза иными, чем записаны. Четыре подписи продукта держались 3,16 · 3,62 ·
 * 3,64 · 4,05 при норме 4,5 — и все четыре стояли в зелёном списке пар.
 *
 * Довод против приёма арифметический, а не вкусовой: белый на полосе `--band`
 * даёт 4,86:1 — это потолок, и уже 92 % белого роняют пару до 4,38.
 * Приглушить текст на полосе и удержать норму нельзя ни при каком значении.
 * Поэтому приём снят целиком, а не настроен.
 */
describe("прозрачность не расходует контраст", () => {
  /* Блок правил: селектор и тело до закрывающей скобки. Вложенных блоков в
     слое нет — медиазапросы разбираются тем же выражением как отдельные
     правила внутри, и это достаточно для обоих правил ниже.
     Комментарии снимаются до разбора, и это не украшение: между закрывающей
     скобкой и селектором в слое стоит объяснение правила, и первая редакция
     захватывала его в селектор — откат назвал нарушителем текст комментария
     вместо имени класса. Замечание, не называющее места, стоит немногим
     больше молчания. */
  const блоки = (css: string): { селектор: string; тело: string }[] =>
    [...css.replace(/\/\*[\s\S]*?\*\//gu, "").matchAll(/([^{}]+)\{([^{}]*)\}/gu)]
      .map((пара) => ({ селектор: (пара[1] ?? "").trim(), тело: пара[2] ?? "" }));

  it.each(OTHER_SHEETS)("$name: текст не носит прозрачности", ({ css }) => {
    const нарушители = блоки(css)
      .filter(({ тело }) => {
        if (!/(^|[;\s])color\s*:/u.test(тело)) return false;
        const мера = /(^|[;\s])opacity\s*:\s*([\d.]+)/u.exec(тело);
        /* `opacity: 1` прозрачностью не является и правилом не задета: она
           стоит там, где отменяет приглушение соседнего правила, — например
           у значка сортировки на устройстве без указателя. Стережётся
           приглушение, а не запись свойства. */
        return мера !== null && Number.parseFloat(мера[2] ?? "1") < 1;
      })
      .map(({ селектор }) => селектор.slice(0, 60));
    expect(нарушители).toEqual([]);
  });

  it.each(OTHER_SHEETS)("$name: полупрозрачный токен не служит фоном", ({ css }) => {
    /* Токены со значением rgba — те, которых проверка контраста не видит:
       она читает шестизначный hex. Заливка, собранная из такого токена,
       не может быть проверена ни при каком списке пар. */
    const прозрачные = new Set(
      [...TOKENS.matchAll(/(--[a-z0-9-]+):\s*rgba\(/gu)].map((пара) => pair1(пара)));
    /* `--overlay` — единственное исключение, и оно записано здесь, а не
       умолчанием: полог лежит поверх фотографии, где контраст не определён
       по природе — снимок бывает любым. Текст на пологе держится собственной
       тенью, а не парой токенов. */
    прозрачные.delete("--overlay");
    const нарушители = блоки(css)
      .flatMap(({ селектор, тело }) =>
        [...тело.matchAll(/background(?:-color)?\s*:\s*var\((--[a-z0-9-]+)\)/gu)]
          .filter((пара) => прозрачные.has(пара[1] ?? ""))
          .map((пара) => `${селектор.slice(0, 40)} → ${pair1(пара)}`));
    expect(нарушители).toEqual([]);
  });
});
...
describe("контраст: 7:1 для основного текста и органов управления, 4,5:1 для подписей", () => {
  ...
  const PAIRS: readonly (readonly [string, string, string, number])[] = [
    ...
    /* Палитра статусов. Комментарий `tokens.css` утверждал, что текст на
       своей заливке держит 7:1 и что «стережёт их design-system.test.ts», —
       а в списке не стояло ни одной такой пары. Комментарий удостоверял
       стража, которого не существовало; числа при этом держались, то есть
       цвет был верен, а гарантии не было. Пары внесены. */
    ["--status-new-ink", "--status-new-soft", "пилюля «Новый»", AAA],
    ["--status-work-ink", "--status-work-soft", "пилюля «В работе»", AAA],
    ["--status-wait-ink", "--status-wait-soft", "пилюля «Ждёт заказчика»", AAA],
    ["--status-pause-ink", "--status-pause-soft", "пилюля «Пауза»", AAA],
    ["--status-done-ink", "--status-done-soft", "пилюля «Готов»", AAA],
    /* Архив своей заливки не имеет намеренно — две серые заливки сошлись бы
       в один вид, — и ложится на нейтральную. Оттого у него четыре фона, а
       не один: пилюля, строка реестра, чётная строка и строка под курсором. */
    ["--status-archive-ink", "--neutral-soft", "пилюля «Архив»", AAA],
    ["--status-archive-ink", "--surface", "статус архива в строке реестра", AAA],
    ["--status-archive-ink", "--surface-2", "статус архива в чётной строке", AAA],
    ["--status-archive-ink", "--accent-soft", "статус архива под курсором", AAA],
    /* Плотные накладки взамен полупрозрачных. Полупрозрачный токен не виден
       проверке по построению: она читает шестизначный hex, и пара с rgba не
       может быть проверена ни при каком списке. Прежние накладки давали 3,64
       и 3,16 — ниже нормы, и никто этого не знал. */
    ["--band-ink", "--band-fill", "блок работающего и колокол на полосе", AA],
    ["--band-ink", "--band-track", "раскрытое «Ещё» на полосе", AA],
    ["--accent-ink", "--accent-fill", "бейдж кода на акцентной обложке", AA],
    ["--accent-ink", "--accent-track", "дорожка кольца на акценте", AA],
  ];
