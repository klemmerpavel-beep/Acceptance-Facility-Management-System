...
/* --- переключатель роли ------------------------------------------------------
   Демонстрация показывает продукт глазами каждой из трёх ролей. До этого
...
await page.waitForTimeout(800);
{
  ...
  if (ролей !== 3) {
    note("роли", `в переключателе ${ролей} ролей вместо трёх`);
  } else {
    const составы = new Map();
    for (const подпись of ["Руководитель", "Прораб", "Заказчик"]) {
