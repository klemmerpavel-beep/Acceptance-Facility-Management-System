...
const строкОбъектов = await число("tbody tr");
if (строкОбъектов < 5) note("проекты", `строк в реестре ${строкОбъектов}`);
...
await раздел("Проекты");
/* Объект открывается ссылкой адреса — ею же его открывает человек. */
