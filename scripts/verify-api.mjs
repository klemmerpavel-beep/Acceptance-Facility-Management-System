...
const черновик = чеки.rows.find((строка) => строка.status === "DRAFT");
check(черновик?.seller === "Всеинструменты", `черновик от «${черновик?.seller}» вместо инструмента`);
check(черновик?.reimbursable === false, "черновик инструмента помечен возмещаемым");
...
check(послеПодтверждения.totals.spent === "5348000",
  `после подтверждения потрачено ${послеПодтверждения.totals.spent} вместо 5348000`);
check(послеПодтверждения.totals.own === "128000",
  `своих расходов ${послеПодтверждения.totals.own} вместо 128000`);
check(послеПодтверждения.totals.drafts === 0,
  `черновиков после подтверждения ${послеПодтверждения.totals.drafts} вместо нуля`);
...
check(послеОтклонения.rows.length === 3,
...
const лентаРуководителя = await owner("/projects/R-99/events?limit=100").then((r) => r.json());
...
check(состояния.size === 3, `на стенде ${состояния.size} состояний денег из трёх: ${[...состояния].join(", ")}`);
...
const ждущие = бухгалтерия.rows.filter((row) => row.state === "ждёт оплаты");
...
for (const row of ждущие) {
  const заПорогом = (row.awaitingDays ?? 0) > бухгалтерия.totals.graceDays;
  check(
    row.overdue === заПорогом,
    `транш ${row.projectCode} № ${row.number}: ждёт ${row.awaitingDays} дней при пороге `
    + `${бухгалтерия.totals.graceDays}, помечен просроченным — ${row.overdue}`,
  );
}
