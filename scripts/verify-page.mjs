...
/* Строк пять. Список короче не оттого, что обещаний стало меньше, а
   оттого, что три из них выполнены: «Заявки» получили свой раздел, «Отчёт»
   — свою вкладку, а график теперь заводится из разделов сметы. Обещать на
   «Что дальше» сделанное значит лгать о составе продукта, поэтому
   сделанное из списка уходит. */
if (roadmap < 5) note("что дальше", `строк ${roadmap} — список неполон`);
