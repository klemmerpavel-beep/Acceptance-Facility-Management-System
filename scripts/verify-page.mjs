...
const строкДенег = await page.locator(".money__row:not(.money__row--client)").count();
if (строкДенег === 0) {
...
/* Строк четыре. Список короче не оттого, что обещаний стало меньше, а
   оттого, что четыре из них выполнены: «Заявки» получили свой раздел,
   «Отчёт» — свою вкладку, график заводится из разделов сметы, а
   «Бухгалтерия» получила свой экран. Обещать на «Что дальше» сделанное
   значит лгать о составе продукта, поэтому сделанное из списка уходит. */
if (roadmap < 4) note("что дальше", `строк ${roadmap} — список неполон`);
