...
/* Сервер спрашивается напрямую там, где нужна ссылка входа: выдаёт её он,
   и класть этот путь через клиента значило бы проверять заодно проксирование. */
...
/**
 * Объект без сметы назван словом на карточке, а не оставлен молчать.
 *
 * Ответ заказчика на вопрос 8 квиза от 14.09.2026: объект без сметы —
 * незавершённое заведение, и продукт обязан называть это на карточке. До
 * ответа карточка печатала «нет» в графе «Смета» тем же видом, каким печатает
 * заполненные графы, — и графа прочитывалась как заполненная.
 *
 * Объект берётся из перечня сервера, а не записывается сюда кодом: коды
 * стенда меняются с каждой правкой посева, и записанный здесь превратился бы
 * в порог, подогнанный под факт.
 */
{
  const безСметыПоСерверу = await page.evaluate(async () => {
    const строки = await fetch("/api/projects", { credentials: "include" })
      .then((ответ) => ответ.json());
    return строки.filter((проект) => проект.estimateTotal === null)
      .map((проект) => проект.code);
  });
  const видимые = (await page.locator(".objecttile .code-badge").allTextContents())
    .map((текст) => текст.trim());
  const ожидаемые = безСметыПоСерверу.filter((код) => видимые.includes(код));
  console.log(`  объектов без сметы в портфеле: ${ожидаемые.length} из ${видимые.length}`);

  /* Правило, которому не на чем упасть, не стережёт ничего: если на стенде не
     осталось ни одного объекта без сметы, об этом говорится прямо, а не
     объявляется успехом. */
  const первый = ожидаемые[0];
  if (первый === undefined) {
    note("карточка объекта",
      "на стенде нет ни одного объекта без сметы: пометку не на чем испытать");
  } else {
    await page.click(`.objecttile:has(.code-badge:text-is("${первый}")) .objecttile__link`);
    await page.waitForSelector(".stamp");
    if ((await page.locator('.stamp .pill:text-is("Сметы нет")').count()) === 0) {
      const графа = ((await page.locator(".stamp__cell", { hasText: "Смета" }).first()
        .textContent()) ?? "").replace(/\s+/gu, " ").trim();
      note("карточка объекта",
        `у объекта ${первый} сметы нет, а карточка об этом не напоминает: графа «${графа}»`);
    }
    await step(`карточка объекта без сметы (${первый})`, "04c-bez-smety.png");
    await page.click('.appbar__link:has-text("Проекты")');
    await page.waitForSelector(".objecttile");
  }
}
...
/* Обратная сторона пометки «сметы нет»: у объекта со сметой её не бывает.
   Пометка, стоящая всюду, ничего не отмечает, а графа «Смета» обязана
   называть редакцию — то единственное, ради чего она заведена. */
{
  const графа = ((await page.locator(".stamp__cell", { hasText: "Смета" }).first()
    .textContent()) ?? "").replace(/\s+/gu, " ").trim();
  if ((await page.locator('.stamp .pill:text-is("Сметы нет")').count()) > 0) {
    note("карточка", `пометка «сметы нет» стоит у объекта R-99 со сметой: графа «${графа}»`);
  }
  if (!/ред\.\s*\d+/u.test(графа)) {
    note("карточка", `графа «Смета» у объекта со сметой не называет редакцию: «${графа}»`);
  }
}
...
/* Группировка «Этап → Помещение → Категория» — второй способ показать те же
   позиции, а не вторая правда о них. Стережётся свойство, а не вид: подвал
   сметы в обеих группировках обязан совпадать до копейки. Разойдясь, они
   означали бы, что одна из группировок теряет позиции молча. */
{
  const итогДоГруппировки = await page.locator("table.estimate tfoot tr").first()
    .locator("td.estimate__num").first().textContent();
  const переключатель = page.locator('[aria-label="Группировка сметы"]');
  if ((await переключатель.count()) === 0) {
    note("смета", "переключателя группировки нет: дерево «Этап → Помещение → Категория» недоступно");
  } else {
    await переключатель.locator('button:has-text("По этапам")').click();
    await page.waitForTimeout(300);
    const строкГруппировки = await page.locator("table.estimate tbody tr").count();
    if (строкГруппировки === 0) {
      note("смета", "группировка по этапам не дала ни одной строки");
    }
    /* Свёрнутой по тому же доводу, что и разделы: три уровня заголовков на
       132 позициях выносят таблицу далеко за порог отрисовки. */
    if (строкГруппировки > 120) {
      note("смета", `группировка открылась развёрнутой: ${строкГруппировки} строк при пороге 120`);
    }
    const итогПослеГруппировки = await page.locator("table.estimate tfoot tr").first()
      .locator("td.estimate__num").first().textContent();
    if (итогДоГруппировки !== итогПослеГруппировки) {
      note("смета", `подвал разошёлся между группировками: «${итогДоГруппировки}» и «${итогПослеГруппировки}»`);
    }
    /* Уровни названы словом: три вложенных заголовка без подписи читаются
       как три раздела одного рода. */
    const подписи = await page.locator("table.estimate tbody .t-cap").allTextContents();
    for (const слово of ["Этап"]) {
      if (!подписи.some((подпись) => подпись.trim() === слово)) {
        note("смета", `в группировке нет уровня «${слово}»: ${подписи.slice(0, 6).join(", ")}`);
      }
    }
    /* Сумма подытогов верхнего уровня равна итогу работ. Правило домена
       проверено тестом; здесь стережётся то, что экран берёт именно его. */
    const этапы = await page.locator("table.estimate tbody tr.estimate__section").count();
    if (этапы === 0) note("смета", "в группировке нет ни одного заголовка уровня");

    await переключатель.locator('button:has-text("По смете")').click();
    await page.waitForTimeout(300);
    console.log(`  смета: группировка по этапам — ${строкГруппировки} строк, подвал сошёлся`);
  }
}
...
/* Подстановка «из обмера» берёт величину ПОМЕЩЕНИЯ позиции, а не итог по
   объекту. Прежде здесь стоял `measure.totals`, и позиции «плитка пола,
   санузел» предлагалось 80,53 м² — площадь пола всей квартиры.

   Числа берутся у самого стенда, а не вписываются: помещение позиции, его
   площадь и итог объекта читаются из API той же сессией. Порог, вписанный
   числом, перестал бы быть правилом на первой же правке наполнения. */
{
  const позицияСПомещением = await page.evaluate(async () => {
    const собрать = (узлы) => узлы.flatMap((узел) => [...узел.items, ...собрать(узел.children)]);
    const вид = await fetch("/api/projects/R-99/estimate", { credentials: "include" })
      .then((r) => r.json());
    const позиция = собрать(вид.sections)
      .find((строка) => строка.room !== null && строка.unit === "м²");
    if (позиция === undefined) return null;
    const обмер = await fetch(`/api/projects/R-99/measure?set=${позиция.room.set}`,
      { credentials: "include" }).then((r) => r.json());
    const комната = (обмер.rooms ?? []).find((строка) => строка.id === позиция.room.id);
    return {
      name: позиция.name,
      room: позиция.room.name,
      floorArea: комната?.floorArea ?? null,
      объект: обмер.totals?.floorArea ?? null,
    };
  });

  if (позицияСПомещением === null) {
    note("правка сметы", "на стенде нет ни одной позиции в м² с помещением: подстановку не на чем проверить");
  } else if (позицияСПомещением.floorArea === позицияСПомещением.объект) {
    note("правка сметы",
      `площадь помещения «${позицияСПомещением.room}» совпала с площадью объекта:`
      + " подмену одной другой на таких данных не различить");
  } else {
    const строка = page.locator("table.estimate tbody tr")
      .filter({ hasText: позицияСПомещением.name }).first();
    if ((await строка.count()) === 0) {
      note("правка сметы", `строка «${позицияСПомещением.name}» не найдена в таблице`);
    } else {
      /* Раздел позиции свёрнут: до правки его надо раскрыть. */
      const раскрытьВсе2 = page.locator('.panel__head .btn--text:has-text("Развернуть все")');
      if ((await раскрытьВсе2.count()) > 0) {
        await раскрытьВсе2.click();
        await page.waitForTimeout(300);
      }
      await строка.locator('.btn--text:has-text("Править")').click();
      await page.waitForTimeout(400);

      const поПомещению = page.locator(".estimate__from-measure")
        .filter({ hasText: "из обмера помещения" });
      if ((await поПомещению.count()) === 0) {
        note("правка сметы",
          `позиции в помещении «${позицияСПомещением.room}» не предложено ни одной величины помещения`);
      } else {
        const подписи = await поПомещению.locator(".btn").allTextContents();
        /* Подпись величины приходит из домена строчными: сверка по написанию
           с заглавной молчала бы на верном коде. */
        const площадь = подписи.find((подпись) => подпись.toLowerCase().includes("площадь пола"));
        const ожидалось = (Number(позицияСПомещением.floorArea) / 1000)
          .toFixed(3).replace(/0+$/, "").replace(/\.$/, "").replace(".", ",");
        if (площадь === undefined || !площадь.replace(/\s|\u00A0/g, "").includes(ожидалось)) {
          note("правка сметы",
            `подстановка помещения «${позицияСПомещением.room}» предложила «${площадь?.trim()}»`
            + ` вместо ${ожидалось} м²: берётся итог по объекту, а не величина помещения`);
        }
      }
      /* Выбор помещения — в том же листе: путь без указателя обязан быть. */
      if ((await page.locator('.sheet select[class~="input"]').count()) === 0) {
        note("правка сметы", "в листе позиции нет выбора помещения");
      }
      await page.click('.sheet .btn--text:has-text("Отмена")');
      await page.waitForTimeout(300);
      console.log(`  правка сметы: помещение «${позицияСПомещением.room}»,`
        + ` площадь ${позицияСПомещением.floorArea} против ${позицияСПомещением.объект} у объекта`);
    }
  }
}

/* Перестановка позиции работает с клавиатуры.
 *
 * Перетаскивание — ускорение, а не единственный путь: у человека без
 * указателя его нет вовсе. Стережётся исход, а не нажатие: после стрелки
 * вниз первая строка раздела обязана стать второй, и обратная стрелка
 * обязана вернуть её на место — иначе проверка оставила бы стенд иным,
 * чем он был, и следующий обход судил бы по чужим числам. */
{
  const раскрыть = page.locator('.panel__head .btn--text:has-text("Развернуть все")');
  if ((await раскрыть.count()) > 0) {
    await раскрыть.click();
    await page.waitForTimeout(300);
  }
  const ручки = page.locator(".estimate__move");
  if ((await ручки.count()) === 0) {
    note("перенос позиции", "ручки перестановки нет ни у одной позиции: перенос недоступен");
  } else {
    const имена = async () => page.locator("table.estimate tbody tr:not(.estimate__section) td:nth-child(2)")
      .allTextContents();
    const доПерестановки = await имена();
    await ручки.first().focus();
    await page.keyboard.press("ArrowDown");
    await page.waitForTimeout(700);
    const послеПерестановки = await имена();
    if (JSON.stringify(доПерестановки) === JSON.stringify(послеПерестановки)) {
      note("перенос позиции", "стрелка вниз не переставила строку: у перетаскивания один путь");
    }
    /* Возврат на место: обход не оставляет следов в порядке сметы. */
    const поднять = page.locator(".estimate__move").nth(1);
    await поднять.focus();
    await page.keyboard.press("ArrowUp");
    await page.waitForTimeout(700);
    const послеВозврата = await имена();
    if (JSON.stringify(доПерестановки) !== JSON.stringify(послеВозврата)) {
      note("перенос позиции", "обратная стрелка не вернула строку: стенд остался переставленным");
    }
    console.log(`  перенос позиции: клавиатурой, строк в разделе ${доПерестановки.length}`);
  }
}
...
/* Импорт называет судьбу помещений.
 *
 * Новая редакция заводит новые строки позиций, и помещения у них пустые по
 * построению: без переноса первый же повторный импорт унёс бы всю
 * расстановку молча. Проверяются три разных утверждения, и ни одно не
 * выводится из другого: связи в базе сохранились, отчёт об этом сказал,
 * и число в отчёте совпало с тем, что в смете.
 *
 * Число берётся из отчёта самого импорта, а не с экрана: разметка отчёта
 * после записи сменяется видом «Импортировано», и правило, читающее её,
 * стерегло бы порядок отрисовки, а не перенос. */
{
  const сведения = await page.evaluate(async () => {
    const собрать = (узлы) => узлы.flatMap((узел) => [...узел.items, ...собрать(узел.children)]);
    const вид = await fetch("/api/projects/R-99/estimate", { credentials: "include" })
      .then((r) => r.json());
    const протоколы = await fetch("/api/projects/R-99/estimate/imports", { credentials: "include" })
      .then((r) => r.json());
    const находка = (протоколы[0]?.report?.findings ?? [])
      .find((строка) => строка.kind === "rooms");
    return {
      вСмете: собрать(вид.sections).filter((строка) => строка.room !== null).length,
      названо: находка?.title ?? null,
    };
  });

  if (сведения.вСмете === 0) {
    note("импорт", "после новой редакции ни одна позиция не сохранила помещение");
  }
  if (сведения.названо === null) {
    note("импорт", "отчёт импорта молчит о судьбе помещений позиций: связи уносятся без слова");
  } else {
    const числа = /(\d+) из (\d+)/u.exec(сведения.названо);
    if (числа === null) {
      note("импорт", `отчёт о помещениях не называет чисел: «${сведения.названо}»`);
    } else if (Number(числа[1]) !== сведения.вСмете) {
      note("импорт",
        `отчёт называет ${числа[1]} перенесённых помещений, в смете их ${сведения.вСмете}`);
    }
  }
  console.log(`  импорт: помещений сохранено у ${сведения.вСмете} позиций`);
}
...
if (шаблоновНаЭкране === 0) {
  ...
  /* Бумага у акта и у документа организации одна, и орган печати у них один.
     Правило повторяется здесь, а не доверяется правилу акта: классы общие,
     а экраны разные, и кнопка пропадает на одном из них молча. */
  await page.evaluate(() => {
    window.__печатьДок = [];
    window.print = () => { window.__печатьДок.push(document.title); };
  });
  const кнопкаДокумента = page.locator('button:has-text("Печать документа")');
  if ((await кнопкаДокумента.count()) === 0) {
    note("документы организации",
      "у выпущенного документа нет кнопки печати: файла заказчик не получит");
  } else {
    await кнопкаДокумента.first().click();
    const имяДокумента = (await page.evaluate(() => window.__печатьДок))[0] ?? "";
    if (!имяДокумента.includes("R-99")) {
      note("документы организации",
        `файл документа сохранится под именем «${имяДокумента}»: объекта в нём не назван`);
    }
  }
  /* Событие шлётся руками: в безголовом прогоне диалога печати нет, и
     `afterprint` сам не придёт. Без него подменённое имя вкладки осталось бы
     стоять до конца обхода — что правило печати акта и обнаружило. */
  await page.evaluate(() => { window.dispatchEvent(new Event("afterprint")); });
  await page.emulateMedia({ media: "print" });
  if (await page.locator(".docs-screen").isVisible()) {
    note("документы организации", "экранная часть раздела попадает в печать документа");
  }
  if (!(await page.locator(".doc.act").first().isVisible())) {
    note("документы организации", "бланк документа в печать не попадает");
  }
...
/**
 * Размер листа собранного PDF.
 *
 * Ориентация страницы не видна ни в разметке, ни в вычисленных стилях: её
 * задаёт `@page`, а он не принадлежит ни одному узлу. Единственный способ
 * замерить её — собрать документ и посмотреть, какого размера вышла
 * страница. `preferCSSPageSize` заставляет сборку слушаться `@page`, иначе
 * размер пришёл бы из настроек по умолчанию и правило мерило бы их.
 */
const размерЛиста = async (кто) => {
  const pdf = await page.pdf({ preferCSSPageSize: true });
  const короба = [...Buffer.from(pdf).toString("latin1")
    .matchAll(/\/MediaBox\s*\[\s*([\d.-]+)\s+([\d.-]+)\s+([\d.-]+)\s+([\d.-]+)\s*\]/gu)]
    .map((найдено) => ({
      ширина: Number(найдено[3]) - Number(найдено[1]),
      высота: Number(найдено[4]) - Number(найдено[2]),
    }));
  const первый = короба[0];
  if (первый === undefined) {
    note(кто, "в собранном PDF не нашлось размера страницы: правило ориентации разоружено");
    return null;
  }
  return { ...первый, страниц: короба.length };
};

/*
 * Акт уходит заказчику файлом.
 *
 * Ответ на вопрос 1 квиза требует акт файлом в двух видах. Решение от
 * 18.09.2026: файл собирает браузер печатью, а не сервер, — сборка PDF на
 * сервере стоила бы новой зависимости и файла шрифта с кириллицей. У такого
 * решения три места, где оно ломается незаметно, и все три замеряются.
 *
 * Первое: имя файла. Браузер берёт его из заголовка вкладки, и без подмены
 * все акты сохранялись бы под именем продукта. Печать подменяется в самом
 * браузере: `window.print()` в безголовом прогоне не открывает ничего, и
 * нажатие без подмены доказало бы лишь, что кнопка не падает.
 *
 * Второе: состав листа. На бумаге обязан остаться бланк и ничего больше.
 *
 * Третье: геометрия. Липкая шапка ведомости встаёт на бумаге поверх первой
 * строки, окно с прокруткой отрезает колонки внутреннего вида, а лист,
 * унаследовавший альбомную страницу обмера, уезжает боком.
 */
{
  await page.evaluate(() => {
    window.__печать = [];
    window.print = () => { window.__печать.push(document.title); };
  });
  const заголовокДо = await page.title();

  /* Наличие органа проверяется до нажатия. Нажатие по отсутствующей кнопке
     роняет обход по истечении ожидания, а падение не называет дефекта: читающий
     отчёт видит «обход не дошёл», а не «кнопки печати нет». */
  const кнопкаАкта = page.locator('button:has-text("Печать акта")');
  const кнопокАкта = await кнопкаАкта.count();
  if (кнопокАкта === 0) {
    note("документы", "у открытого акта нет кнопки печати: файла заказчик не получит");
  }
  if (кнопокАкта > 0) await кнопкаАкта.first().click();
  await page.click('.segmented__option:has-text("Внутренний")');
  await page.waitForSelector('.act__table thead th:has-text("Прибыль")', { timeout: 5000 })
    .catch(() => undefined);
  if ((await кнопкаАкта.count()) > 0) await кнопкаАкта.first().click();
  const имена = await page.evaluate(() => window.__печать);
  ...
  for (const [индекс, вид] of (кнопокАкта === 0 ? [] : ["клиентский", "внутренний"]).entries()) {
    ...
    if (!имя.includes("R-99")) {
      note("документы", `в имени файла акта (${вид} вид) нет объекта: «${имя}»`);
    }
    if (!имя.endsWith(вид)) {
      note("документы", `имя файла не называет вид акта: «${имя}» вместо оканчивающегося на «${вид}»`);
    }
  }
  /* Имя вкладки возвращается: подменённый заголовок остался бы стоять в
     закладках и в истории браузера, назвав весь продукт одним актом. */
  await page.evaluate(() => { window.dispatchEvent(new Event("afterprint")); });
  const заголовокПосле = await page.title();
  if (заголовокПосле !== заголовокДо) {
    note("документы",
      `после печати имя вкладки осталось «${заголовокПосле}» вместо «${заголовокДо}»`);
  }

  await page.emulateMedia({ media: "print" });
  if (await page.locator(".acts-screen").isVisible()) {
    note("документы", "экранная часть вкладки попадает в печать акта");
  }
  if (!(await page.locator(".act").first().isVisible())) {
    note("документы", "бланк акта в печать не попадает");
  }
  for (const [узел, имя] of [[".appbar", "шапка"], [".tabs", "полоса вкладок"]]) {
    if (await page.locator(узел).isVisible()) {
      note("документы", `${имя} попадает в печать акта`);
    }
  }
  /* Мерится липкость, а не повтор шапки: повтор — умолчание браузера для
     `thead`, и правило на него не может упасть. Правило, которое нельзя
     сломать, не стережёт ничего — откат это и показал. */
  const шапкаВедомости = await page.locator(".act .estimate thead").first()
    .evaluate((узел) => getComputedStyle(узел).position);
  if (шапкаВедомости === "sticky") {
    note("документы", "шапка ведомости акта на печати липкая: она встанет поверх первой строки");
  }
  const окно = await page.locator(".act .table-scroll").first()
    .evaluate((узел) => getComputedStyle(узел).overflowX);
  if (окно !== "visible") {
    note("документы", `окно ведомости акта на печати «${окно}»: колонки уедут за край листа`);
  }
  const лист = await размерЛиста("документы");
  if (лист !== null && лист.ширина >= лист.высота) {
    note("документы",
      `первый лист акта альбомный: ${String(Math.round(лист.ширина))}×${String(Math.round(лист.высота))} pt`
...
/* Обратная сторона правила ориентации: ведомость помещений остаётся
   альбомной. Книжной страницы ей мало — в ведомости семь колонок, и на
   книжном листе они сжимаются до нечитаемого. Именованная страница `plan`
   действует только пока класс её просит, а просьба живёт в одной строке
   стилей и снимается незаметно. */
{
  const лист = await размерЛиста("замер");
  if (лист !== null && лист.ширина <= лист.высота) {
    note("замер",
      `первый лист обмера книжный: ${String(Math.round(лист.ширина))}×${String(Math.round(лист.высота))} pt`
      + ` страниц в файле: ${String(лист.страниц)}`);
  }
}
 ...
 * единиц измерения закрыт ему, а его отказ стоял под общим `try` загрузки и
 * объявлял вошедшего неизвестным. Вход был, продукта не было.
 *
 * Стережётся не вид экрана, а три свойства, каждое из которых уже было
 * нарушено:
 *   1. вошедший стоит на разделе, который есть в его навигации;
 *   2. на первом экране нет первичного действия, которого роль не выполнит;
 *   3. вход не тратит ни одного отказанного запроса — отказ в журнале
 *      сервера неотличим от попытки залезть не в своё.
 *
 * Вход идёт по ссылке, выданной сервером, — тем же путём, каким руководитель
 * выдаёт доступ в продукте. Второго способа входа ради проверки не заводится.
 * -------------------------------------------------------------------------- */
const РОЛИ_ВХОДА = [
  {
    имя: "прораб",
    почта: "foreman@dolgiy.studio",
    разделы: ["Главная", "Заявки", "Проекты", "Контакты", "Бухгалтерия"],
    вкладки: "Обзор|Замер|Смета|Работа|Приёмка|Чеки|Отчёт|Транши|Документы",
    снимок: "48-vhod-prorab.png",
  },
  {
    имя: "заказчик",
    почта: "client@dolgiy.studio",
    разделы: ["Проекты"],
...
for (const роль of РОЛИ_ВХОДА) {
  ...
  if (typeof ссылка.token !== "string") {
    note("вход по ролям", `${роль.имя}: сервер не выдал ссылку входа`);
    continue;
  }

  const свой = await browser.newContext({
    viewport: { width: 1440, height: 900 }, locale: "ru-RU",
  });
  const лист = await свой.newPage();
  /* Обмен идёт через тот же узел, что и весь обход, и возврат — на него же.
     Сервер переадресует на `WEB_ORIGIN`, а это другое имя того же места:
     кука сессии ставится на узле обмена и на узле переадресации не видна.
     Возврат на свой узел — не обход правила, а возврат туда, где кука есть. */
  await лист.goto(`${BASE}/api/auth/consume?token=${ссылка.token}`, { waitUntil: "networkidle" })
    .catch(() => { /* переадресация на чужой узел — ожидаемо */ });
  /* Счёт отказов начинается здесь, после обмена: заход на узел
     переадресации идёт без куки и отвечает отказом по устройству двух имён
     одного места, а не по вине продукта. Считать его значило бы стеречь
     настройку стенда вместо поведения роли. */
  const отказано = [];
  лист.on("response", (отклик) => {
    if (отклик.status() === 403 || отклик.status() === 401) {
      отказано.push(new URL(отклик.url()).pathname);
    }
  });
  await лист.goto(BASE, { waitUntil: "networkidle" });

  /* Ожидание ограничено и оканчивается замечанием, а не падением. Именно
     этот исход и стерегут: заказчик не входил вовсе — его выбрасывало
     обратно на форму входа, — и правило, падающее по истечении ожидания,
     назвало бы это «проверка сломалась», а не «роль не входит». */
  const вошёл = await лист.waitForSelector(".appbar", { timeout: 12_000 })
    .then(() => true).catch(() => false);
  if (!вошёл) {
    const наЭкране = ((await лист.locator("h1").first().textContent()) ?? "—").trim();
    note("вход по ролям",
      `${роль.имя} не вошёл: после обмена ссылки на экране «${наЭкране}»`);
    await свой.close();
    continue;
  }
  await лист.waitForTimeout(600);

  const разделы = (await лист.locator(".appbar__nav-scroll .appbar__link").allTextContents())
    .map((текст) => текст.trim());
  if (разделы.join("|") !== роль.разделы.join("|")) {
    note("вход по ролям", `${роль.имя}: разделы «${разделы.join(", ")}»`);
  }

  const заголовок = ((await лист.locator(".cover h1").first().textContent().catch(() => "")) ?? "")
    .trim();
  if (!роль.разделы.includes(заголовок)) {
    note("вход по ролям",
  ...
  await лист.waitForTimeout(600);
  const плитка = лист.locator(".objecttile__link").first();
  if ((await плитка.count()) === 0) {
    note("вход по ролям", `${роль.имя}: в перечне объектов пусто — смотреть нечего`);
  } else {
    await плитка.click();
    await лист.waitForTimeout(1400);
    const вкладки = (await лист.locator(".tabs__item").allTextContents())
      .map((текст) => текст.trim());
    if (вкладки.join("|") !== роль.вкладки) {
      note("вход по ролям", `${роль.имя}: вкладки карточки «${вкладки.join(", ")}»`);
    }
